import os
import random
from apps.Optics.classes import images
import pygame
from apps.Optics.gui import button
from apps.Optics import settingsSetup
from apps.Optics.screens import settings_screen, loading_saves_screen, achievements_screen, music_settings
from apps.Optics.classes import parkinson as particles
from apps.Optics.gui.button_animation import ButtonAnimation
from apps.Optics.classes import mixer_c
    ...
    def delete(self):
        if self.executed_functions != 'delete':
            for idx, (key, value) in enumerate(self.selected_buttons.items()):
                if value == True:
                    os.remove(f'apps/Optics/saves/{key}.json')
