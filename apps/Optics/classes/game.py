...
# from math import *
from pygame import *
from apps.Optics.gui import gui_main as gui1
from apps.Optics.gui.gui_main import GUI
from apps.Optics.screens import settings_screen, achievements_screen, music_settings
from apps.Optics import settingsSetup
from apps.Optics.classes import fps
from apps.Optics.classes import bin, images, gameobjects
from apps.Optics.classes.achievements import Achievements
from apps.Optics.classes import parkinson as particles
from apps.Optics.classes.font import Font
import time
from datetime import datetime
from apps.Optics import functions
from apps.Optics import gui
from apps.Optics.gui.polygonDrawing import polygonDrawing
from apps.Optics.classes import popup
from apps.Optics.classes import saveTK, mixer_c
...
class Game:
    ...
    def load(self):
        if not self.preset:
            try:
                with open(f"apps/Optics/saves/{self.save_to_load}.json", 'r') as f:
                    save = json.load(f)
                    f.close()
            except:
                save = {}
            ...
            try:
                with open(f"apps/Optics/presets/{self.save_to_load}.json", 'r') as f:
                    save = json.load(f)
                    f.close()
            except:
                save = {}
    ...
    def save_to_file(self):
        if os.path.exists(f'apps/Optics/saves/{self.save_title}'):
            old_save = settingsSetup.load_settings(f'apps/Optics/saves/{self.save_title}.json')
            new_save = old_save.update(self.save_obj)
            settingsSetup.writesettingstofile(new_save, 2, f'apps/Optics/saves/{self.save_title}.json')
        else:
            settingsSetup.writesettingstofile(self.save_obj, 2, f'apps/Optics/saves/{self.save_title}.json')
        ...
        if not self.preset:
            ...
            if len(self.objects) > 3:
                ...
                if self.save_title != None:
                    prev_save_data = settingsSetup.load_settings(f'apps/Optics/saves/{self.save_title}.json')
